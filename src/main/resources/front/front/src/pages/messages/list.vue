<template>
  <div
    ...
    <el-form ref="form" :model="form" :rules="rules" label-position="left" label-width="65px">
	  ...
	  <span v-for="item in infoList" :key="item.id">
        ...
        <div v-if="item.cpicture" class="content" style="margin: 0;flex: 1;">
            <img :src="$config.baseUrl+item.cpicture"
                 style="max-width: 300px;max-height: 300px;border: 2px solid #EEEEEE;margin:8px 0 0 50px;">
        </div>
		<div v-if="item.reply" class="content-block-reply">
		  回复：{{ item.reply }}
		</div>
